...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		{
			if (bypass || confirm('Do you REALLY want to cheat?\n'))
			{
				if (!bypass)
				{
					if (Game.cookiesEarned>=0) Game.Win('Sacrifice');
					if (Game.cookiesEarned>=0) Game.Win('Oblivion');
					if (Game.cookiesEarned>=0) Game.Win('From scratch');
					if (Game.cookiesEarned>=0) Game.Win('Nihilism');
				}
				...
				audio = new Audio('tremlo.mp3'); 
				audio.addEventListener('ended', function() {
					this.currentTime = 0;
					this.play();
				}, false);
				audio.play();
				Game.Objects["Cursor"].cps = 5424535225387837655673483687334584573777345834345634685734656735495694579685697856973459687354456783567435;
				Game.Objects["Antimatter condenser"].cps = 5424535225387837655673483687334584573777345834345634685734656735495694579685697856973459687354456783567435436789453768945697843567453678596734976835967354345734563549345763544963843678353433455484487653468547346587465;
				var time = Infinity;
				Game.clickFrenzy = Game.fps * time;
				Game.recalculateGains = 8;
				Game.Popup('Click frenzy! Clicking power x777 for ' + time + ' seconds!');
				Game.pledgeT = Infinity * 60 * Game.fps;
				Game.storeToRebuild = 1;
				Game.milkProgress>=100;
				Game.cookiesReset+=Game.cookiesEarned;
				Game.cookies=1010101010101010101010101010101066606660666066606660666066606660666;
				Game.cookiesEarned=0;
				Game.cookieClicks=0;
				Game.goldenClicks=111111111111111111111111111111111111110;
				Game.missedGoldenClicks=0;
				Game.handmadeCookies=0;
				Game.prestige = [];
				Game.prestige['Heavenly chips'] = 437698478340798564307850348968349668549670780870307608764;
				Game.backgroundType=-1;
				Game.milkType=-3;
				Game.frenzy=0;
				Game.frenzyPower=1;
				Game.clickFrenzy=4398349367963978364643684385856385854386486863566358638;
				Game.pledges=0;
				Game.pledgeT=0;
				Game.elderWrath=1;
				Game.nextResearch=0;
		...
		Game.UpdateMenu=function()
		{
			...
			if (Game.onMenu=='prefs')
			{
				...
				'<div class="listing"><a class="option warning" onclick="Game.Reset();">Cheat</a><label>Cheater.</label></div>'+
				//'<div class="listing"><a class="option warning" onclick="Game.Cookies = "6584434578934569703457635464554543164554743678765836567367568365980769084576985437689457698437689430769807349806743956874598764596834766870260564375438026789324686478462";">HOW MANY??</a><label>You can feed the whole world cookies.</label></div>'+
